...
const codes = fs.readFileSync('/tmp/codes.txt','utf8').split('\n').filter(Boolean);
const nameToCode: Record<string,string> = {};
for (const line of fs.readFileSync('/tmp/name_to_code.tsv','utf8').split('\n').filter(Boolean)) {
...
for (const f of files) {
  ...
  const blocks = parseAnchors(raw, codes, nameToCode);
