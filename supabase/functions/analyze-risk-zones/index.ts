...
1. ПРИОРИТЕТ: ссылки на конкретные биомаркеры из анализов (например: "CRP = 5.2 мг/л — выше нормы", "Витамин D = 18 нг/мл — ниже оптимума").
2. РАЗРЕШЕНО также: симптомы пациента с severity (например: "Качество сна severity=2/5"), данные из медицинской истории, динамика веса.
3. СТРОГО ЗАПРЕЩЕНО: процент соблюдения назначений (adherence %), названия конкретных назначений/рецептов, текст вида "Среднее соблюдение назначения '...' = X%".
4. Блокер должен быть подкреплён хотя бы одним фактом из данных пациента (биомаркер, симптом или мед.история).`;
          ...
          blockers: {
            ...
            items: {
              ...
              properties: {
                ...
                evidence: { type: "array", items: { type: "string" }, description: "Факты из данных пациента: биомаркеры (приоритет), симптомы с severity, мед.история. ЗАПРЕЩЕНЫ: adherence %, названия назначений." },
