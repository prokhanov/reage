...
Используй свою экспертизу для тонкой корректировки на основе синергий маркеров, симптомов и динамики.`;
    ...
    // Параллельно генерируем рекомендации в новой системе v2 (fire-and-forget).
    // Ошибки v2 не должны влиять на основной отчёт.
    try {
      const v2Url = `${supabaseUrl}/functions/v1/analyze-biomarkers-v2`;
      fetch(v2Url, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        ...
        .then((r) => r.text())
        .then((t) => console.log("[v2-trigger] response:", t.substring(0, 200)))
        .catch((e) => console.error("[v2-trigger] failed:", e));
    } catch (e) {
      console.error("[v2-trigger] schedule failed:", e);
    }
