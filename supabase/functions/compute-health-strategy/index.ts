...
СИСТЕМЫ для goals: ${systemNames.join(", ")}`;
    ...
    // System ratings (per-category) from latest analysis AI metadata + deviation counts
    const aiCategoryScores: Record<string, number> = (latest.biomarkers_metadata as any)?.ai_analysis?.category_scores || {};
    const systemRatings = (categories as any[]).map((c) => {
      const items = byCat[c.name] || [];
      const total = items.length;
      const deviated = items.filter((b) => b.deviated).length;
      const score = aiCategoryScores[c.name];
      let band = "—";
      ...
      const rationale = total === 0
        ...
        : `${deviated} из ${total} маркеров вне оптимума${typeof score === "number" ? `, оценка ${score}/100 (${band})` : ""}`;
      return {
        category: c.name,
    ...
    const explanation = {
      ...
      health_index: {
        ...
        breakdown: [
          ...
          `Формула: 100 − (0.6·среднее_отклонение + 0.4·топ-5_штраф)·12, бонус за ≥80% оптимальных`,
        ],
      },
      ...
      drivers: [
        ...
        `Базовый био-возраст: ${baseBioAge.toFixed(1)} = ${chronoAge} + (85 − ${hiVal}) × 0.25`,
