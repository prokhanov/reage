...
serve(async (req) => {
  ...
  try {
    ...
    if (claimsErr || !claimsData?.claims?.sub) {
      console.error("[compute-health-strategy] getClaims failed", claimsErr?.message);
    ...
    const { data: promptRows } = await supabase
      ...
      .in("key", ["health_strategy_system", "health_strategy_user"]);
    const promptMap = new Map((promptRows || []).map((p: any) => [p.key, p.prompt_text]));

    const promptVars: Record<string, string> = {
      intervalMonths: String(intervalMonths),
      patientCodes: patientCodes.join(", ") || "—",
      prescTitles: prescTitles.join("; ") || "(назначений нет — оставь массив пустым)",
      prescTitlesOrLifestyle: prescTitles.join("; ") || "(назначений нет — опирайся на образ жизни)",
      milestonesCount: String(milestonesCount),
      requiredSlotsText,
      deviatedCodes: deviatedCodes.join(", ") || "—",
      complaintsText,
      startDate: toIso(startDate),
      chronoAge: String(chronoAge),
      currentBio: String(currentBio),
      gender: profile.gender === "female" ? "женский" : "мужской",
      planName: planName || "не указан",
      analysesPerYear: String(analysesPerYear),
      adherence: adherencePct != null ? adherencePct + "%" : "нет данных",
      analysisDates: finalAnalysisDates.join(", ") || "нет в рамках года",
      doctorReviewDate: toIso(doctorReviewDate),
      categoriesContext,
      chronicText,
      anketaContext: await buildAnketaContextAsync(supabase, profile),
      prescContext: prescContext || "(нет активных назначений — действия должны строиться вокруг наблюдения и образа жизни)",
      systemNames: systemNames.join(", "),
    };

    const systemPrompt = renderPrompt(
      promptMap.get("health_strategy_system") || DEFAULT_HEALTH_STRATEGY_SYSTEM,
      promptVars,
    );
    const userPrompt = renderPrompt(
      promptMap.get("health_strategy_user") || DEFAULT_HEALTH_STRATEGY_USER,
      promptVars,
    );
