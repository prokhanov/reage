...
serve(async (req) => {
  ...
  try {
    ...
    // Промежуточные контрольные точки между анализами (помесячно)
...
4) ГОДОВУЮ КАРТУ ПУТИ (roadmap) — ровно ${milestonesCount} майлстоунов с шагом ~1.5 месяца. Структура:
   • Первый (kind=start, date=${toIso(startDate)}) — «Стартовая точка»: перечисли 3-4 КОНКРЕТНЫХ отклонения с кодами и текущими значениями (например «Витамин D 18 нг/мл — дефицит», «СРБ 4.2 мг/л — субклиническое воспаление»).
   • Промежуточные (kind=milestone) — это НЕ анализы, а конкретные контрольные точки: старт приёма препаратов, оценка переносимости (2-3 нед), коррекция образа жизни (сон/питание/нагрузка), промежуточная самооценка симптомов, подготовка к следующему анализу.
   • Анализы (kind=analysis) в даты: ${finalAnalysisDates.slice(0, -1).join(", ") || "—"} — «Контрольный анализ №N»: какие маркеры пересдаём и почему, какое решение принимаем по итогам.
   • Последний (kind=summary, date=${finalAnalysisDates[finalAnalysisDates.length - 1]}) — «Итоги года»: сравнение стартовых и финальных значений по ключевым кодам, пересчёт биовозраста, план на следующий год.

   ТРЕБОВАНИЯ к каждому майлстоуну:
   - title: КОНКРЕТНЫЙ (например «Старт Витамин D 5000 МЕ», «Оценка сна и энергии», «Контроль ферритина и B12», «Коррекция дозировок»), НЕ «Первичная оценка», НЕ «Устойчивая динамика», НЕ «Промежуточная проверка».
   - description: 1 строка, что именно происходит в этой точке.
   - bullets: 3-4 пункта. Каждый пункт = конкретное действие/измерение с КОДОМ биомаркера или НАЗВАНИЕМ препарата или конкретной жалобой пациента. Примеры хороших пунктов: «Начать Магний цитрат 400 мг вечером — против бессонницы», «Через 3 нед оценить переносимость и сон по дневнику», «Пересдать 25(OH)D — цель >40 нг/мл», «Если ferritin <50 — добавить железо».
   - focus: до 40 символов, конкретная цель этапа с маркером (например «D ↑ до 40 нг/мл», «снизить СРБ <1»).
   - ЗАПРЕЩЕНО: «улучшение самочувствия», «работа над здоровьем», «первичная оценка», «устойчивая динамика», «системный подход», «комплексная работа», любые общие слова без цифр/кодов/названий.
