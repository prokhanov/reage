-- Default template for confirmed booking (only if absent)
UPDATE public.telegram_notification_settings
SET booking_templates = COALESCE(booking_templates, '{}'::jsonb)
  || jsonb_build_object(
       'booking_scheduled',
       E'✅ <b>Заявка подтверждена</b>\n👤 {patient}\n📱 {phone}\n📧 {email}\n🗓 {date} {time}\n📍 {address}\n🏷 Статус: <b>{status}</b>'
     )
WHERE singleton = true
  AND NOT (COALESCE(booking_templates, '{}'::jsonb) ? 'booking_scheduled');

-- Trigger function fired on status change
...
BEGIN
  ...
  -- Skip statuses we don't have a template for (e.g. not_scheduled, no_answer)
...
AFTER UPDATE OF status ON public.analysis_bookings
