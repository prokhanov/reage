...
BEGIN
  ...
  INSERT INTO public.profiles (
    id, name, first_name, last_name, email, birth_date, gender,
    weight, height, phone, operations, medications, health_note, email_verified
  ) VALUES (
    NEW.id, profile_name,
    COALESCE(NULLIF(metadata->>'first_name', ''), profile_name),
    NULLIF(metadata->>'last_name', ''),
    NEW.email, birth_date_value, gender_value, weight_value, height_value,
    NULLIF(metadata->>'phone', ''),
    COALESCE(metadata->'operations', '{}'::jsonb),
    medications_value,
    NULLIF(metadata->>'health_note', ''),
    false  -- ВСЕГДА false при регистрации; подтверждается только кликом по нашей ссылке
  )
  ON CONFLICT (id) DO UPDATE SET
    ...
    -- email_verified НЕ перезаписываем при upsert, чтобы не сбросить подтверждённый флаг
    updated_at = now();
  ...
  IF plan_data <> '{}'::jsonb AND COALESCE((plan_data->>'skipPayment')::boolean, false) = false THEN
    ...
    INSERT INTO public.subscriptions (
      user_id, status, plan_id, pricing_id, plan_type, amount,
      ...
      amount_value, now(), now() + make_interval(months => duration_months), 'card'
    );
  ELSE
