// Runs before `vite dev` and `vite build` (predev/prebuild hooks); writes public/sitemap.xml.
import { writeFileSync } from "fs";
import { resolve } from "path";

const BASE_URL = "https://reage.life";

interface SitemapEntry {
  path: string;
  changefreq?: "always" | "hourly" | "daily" | "weekly" | "monthly" | "yearly" | "never";
  priority?: string;
}

const entries: SitemapEntry[] = [
  { path: "/", changefreq: "weekly", priority: "1.0" },
  { path: "/monitoring", changefreq: "weekly", priority: "0.9" },
  { path: "/checkup", changefreq: "weekly", priority: "0.9" },
  { path: "/example-report", changefreq: "weekly", priority: "0.8" },
  { path: "/prep", changefreq: "monthly", priority: "0.7" },
  { path: "/business", changefreq: "monthly", priority: "0.8" },
  { path: "/checkup/full", changefreq: "weekly", priority: "0.9" },
  { path: "/checkup/energy", changefreq: "weekly", priority: "0.9" },
  { path: "/checkup/thyroid", changefreq: "weekly", priority: "0.8" },
  { path: "/checkup/iron", changefreq: "weekly", priority: "0.8" },
  { path: "/checkup/cardio-risk", changefreq: "weekly", priority: "0.8" },
  { path: "/checkup/metabolic", changefreq: "weekly", priority: "0.8" },
  { path: "/checkup/liver", changefreq: "weekly", priority: "0.8" },
  { path: "/checkup/kidney", changefreq: "weekly", priority: "0.8" },
  { path: "/checkup/base", changefreq: "weekly", priority: "0.8" },
  { path: "/checkup/vitamins", changefreq: "weekly", priority: "0.8" },
  { path: "/checkup/female-hormones", changefreq: "weekly", priority: "0.8" },
  { path: "/checkup/male-hormones", changefreq: "weekly", priority: "0.8" },
  { path: "/checkup/hair", changefreq: "weekly", priority: "0.8" },
];

const xml = [
  `<?xml version="1.0" encoding="UTF-8"?>`,
  `<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">`,
  ...entries.map((e) =>
    [
      `  <url>`,
      `    <loc>${BASE_URL}${e.path}</loc>`,
      e.changefreq ? `    <changefreq>${e.changefreq}</changefreq>` : null,
      e.priority ? `    <priority>${e.priority}</priority>` : null,
      `  </url>`,
    ]
      .filter(Boolean)
      .join("\n"),
  ),
  `</urlset>`,
].join("\n");

writeFileSync(resolve("public/sitemap.xml"), xml);
console.log(`sitemap.xml written (${entries.length} entries)`);
