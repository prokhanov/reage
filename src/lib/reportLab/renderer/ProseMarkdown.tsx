...
export function ProseMarkdown({ markdown, className = "", editableId }: Props) {
  ...
  const clean = source
    ...
    // Ловим в любом месте абзаца (в т.ч. как последнее предложение после
    // длинного текста), допускаем дефис-перенос «найдё-те».
    .replace(
      /(Все\s+рекомендации\s+по\s+выявленным\s+показателям[^\n]*?вы\s+найд[её]-?те\s+в\s+разделе\s+«?)(?:Назначения|Рекомендации)(»?\.?)/gi,
      (_m, pre: string, post: string) => `*${pre.replace(/-\s*/g, "")}Рекомендации${post}*`,
    )
    .replace(
      /(Рекомендации\s+по\s+коррекции[^\n]*?вы\s+найд[её]-?те\s+в\s+разделе\s+«?)(?:Назначения|Рекомендации)(»?\.?)/gi,
      (_m, pre: string, post: string) => `*${pre.replace(/найд[её]-\s*те/gi, (s) => s.replace(/-\s*/, ""))}Рекомендации${post}*`,
    )
    .trim();
