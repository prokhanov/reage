...
export function ProseMarkdown({ markdown, className = "", editableId }: Props) {
  ...
  const clean = source
    ...
    // ВАЖНО: без флага `i` — иначе `[А-ЯЁ]` в lookahead ловит и строчные буквы
    // и рвёт заголовок ровно посередине (напр. «Общая оценка| системы организма»).
    .replace(
      new RegExp(
        `(^|\\n)[ \\t]*(?:#{1,6}[ \\t]+)?\\*{2}(${SUMMARY_SUBHEADING_RE})\\*{2}[ \\t]*(?=\\n|$)`,
        "g",
      ),
    ...
    .replace(
      new RegExp(
        `(^|\\n)[ \\t]*(?:#{1,6}[ \\t]+|\\*{1,2})?(${SUMMARY_SUBHEADING_RE})(?:\\*{1,2})?[ \\t]*[:：—–-][ \\t]*`,
        "g",
      ),
      (_m, nl: string, title: string) => `${nl}\n**${title.trim()}**\n\n`,
    )
    ...
    // Разворачиваем возможные переносы строк внутри самой фразы, чтобы
    // regex ниже мог её сопоставить и обернуть в курсив без «залипших» `*`.
    .replace(
      /(Все\s+рекомендации\s+по\s+выявленным\s+показателям[\s\S]{0,300}?вы\s+найд[её]-?\s*те\s+в\s+разделе\s+«?(?:Назначения|Рекомендации)»?\.?)/gi,
      (m: string) => m.replace(/\s+/g, " "),
    )
    .replace(
      /(Рекомендации\s+по\s+коррекции[\s\S]{0,200}?вы\s+найд[её]-?\s*те\s+в\s+разделе\s+«?(?:Назначения|Рекомендации)»?\.?)/gi,
      (m: string) => m.replace(/\s+/g, " "),
    )
    .replace(
    ...
    // Убираем «пустые» пункты списка (одиночный маркер без содержимого),
    // которые иногда остаются после схлопывания абзацев.
    .replace(/^[ \t]*[-*•][ \t]*$/gm, "")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
