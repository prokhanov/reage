...
`;
  ...
  it("не переносит ручной текст из MONO обратно в MONO-ABS при сохранении", () => {
    const bios = [
      { ...mkBio("MONO", "Моноциты"), value: 8, unit: "%" },
      { ...mkBio("MONO-ABS", "Моноциты (абс.)"), value: 0.44, unit: "×10⁹/л" },
    ];
    const index = buildBiomarkerIndex(mkReport(bios));
    const text = `
Воспалительная и иммунная система

<!-- anchor:biomarker MONO -->
Моноциты — это «уборщики» иммунной системы. Они поглощают остатки клеток, патогены и участвуют в регуляции воспаления.

Ваш показатель 8 % находится в оптимальном диапазоне. Это говорит об отсутствии признаков активного хронического воспаления.
<!-- anchor:biomarker_end -->

<!-- anchor:biomarker MONO-ABS -->

<!-- anchor:biomarker_end -->
`;

    const parsed = parseCategory("Воспалительная и иммунная система", text, index);
    ...
    expect(byCode.get("mono")).toContain("Моноциты — это «уборщики»");
    expect(byCode.get("mono")).toContain("Ваш показатель 8 %");
    expect(byCode.get("monoabs") || "").toBe("");
  });
});
