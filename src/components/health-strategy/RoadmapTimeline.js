...
export function RoadmapTimeline({ startDate, nextCheckupDate, roadmap, keyBiomarkers, analysesPerYear, adherencePct }) {
    const { format: formatMarker } = useBiomarkerNames();
    const today = new Date();
    const start = new Date(startDate);
    // Fallback if roadmap not yet generated
    const milestonesRaw = (roadmap && roadmap.length > 0)
        ? roadmap
        : [
    ...
    const cleanTitle = (title, num, kind) => {
        ...
        let next = title
            ...
            .trim();
        if (!next && num)
            next = "Плановая сдача анализов";
        return next || title;
    };
    const cleanBullets = (bullets, kind) => {
    ...
    return (<Card className="border-border bg-card overflow-hidden">
      <CardContent className="p-4 md:p-6 space-y-5 md:space-y-6">
        ...
        <div className="flex items-end justify-between gap-3 flex-wrap">
          ...
          {adherencePct != null && (<Badge variant="secondary" className="text-xs">
        ...
        <div className="overflow-x-auto md:overflow-visible -mx-2 px-2">
          <div className="relative min-w-[720px] md:min-w-0" style={{ height: 260 }}>
            ...
            <svg viewBox="0 0 1000 260" preserveAspectRatio="xMidYMax slice" className="absolute inset-0 w-full h-full pointer-events-none select-none" aria-hidden>
              <defs>
              ...
              <path d="M0,260 L0,175 Q90,170 160,178 Q230,150 320,168 Q410,185 500,155 Q590,125 690,145 Q790,120 890,138 Q970,150 1000,130 L1000,260 Z" fill="url(#mtFar)" filter="url(#mtBlurHeavy)"/>

              {/* Mid ridge layer */}
              <path d="M0,260 L0,195 Q70,190 130,200 Q210,175 290,192 Q370,205 450,185 Q530,160 630,180 Q730,155 830,175 Q910,190 1000,170 L1000,260 Z" fill="url(#mtMid)" filter="url(#mtBlur)"/>

              {/* Near ridge layer */}
              <path d="M0,260 L0,218 Q60,212 120,222 Q200,198 280,215 Q360,228 440,208 Q520,188 620,203 Q720,182 820,200 Q900,212 1000,198 L1000,260 Z" fill="url(#mtNear)" filter="url(#mtBlur)"/>

              {/* Soft valley floor */}
              <path d="M0,260 L0,238 Q120,234 260,236 Q500,232 740,236 Q880,234 1000,238 L1000,260 Z" fill="url(#mtValley)"/>

              {/* Faint winding trail */}
              <path d="M0,230 C 120,226 180,238 260,230 C 340,222 400,240 500,232 C 600,224 680,238 780,230 C 860,222 930,228 1000,224" fill="none" stroke="hsl(var(--muted-foreground))" strokeOpacity="0.18" strokeWidth="1" strokeDasharray="2 5" vectorEffect="non-scaling-stroke"/>
            </svg>
            ...
            return (<div key={i} className="absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center" style={{ left: `${toLeftPct(p.x)}%`, top: `${toTopPct(p.y)}%` }}>
                  {/* Date + title positioned opposite the curve direction */}
                  {above ? (<div className="absolute bottom-full mb-2 w-[140px] text-center">
                  ...
                  {!above ? (<div className="absolute top-full mt-2 w-[140px] text-center">
                      <div className="text-[10px] text-muted-foreground tabular-nums whitespace-nowrap">
                        {format(date, "d MMM yyyy", { locale: ru })}
                      </div>
                      <div className={`text-[11px] md:text-xs font-semibold leading-tight mt-0.5 line-clamp-2 ${i > activeIdx ? "text-muted-foreground" : "text-foreground"}`}>
          ...
          {milestones.map((m, i) => {
            ...
            return (<div key={i} className={[
                ...
                <ul className="space-y-1 mt-1">
                  {(m.bullets || []).slice(0, 4).map((b, j) => (<li key={j} className="text-xs text-foreground/80 leading-snug flex gap-1.5">
                      <span className="text-primary mt-1">•</span>
                      <span>{b}</span>
                    </li>))}
                </ul>
                {current && adherencePct != null && (<div className="pt-1">
                ...
                {m.focus && (<div className="mt-auto pt-2">
        ...
        {/* Key biomarkers */}
        {keyBiomarkers && keyBiomarkers.length > 0 && (<div className="rounded-lg border border-primary/20 bg-primary/[0.03] p-4 space-y-3">
