  ...
  return (
    <Card className="bg-card/40 backdrop-blur-xl dark:border-white/10 border-border/60 dark:shadow-none shadow-xl shadow-muted-foreground/60">
      <CardHeader>
        ...
        <p className="text-xs dark:text-white/50 text-muted-foreground">Что закрывает ваши главные риски</p>
      </CardHeader>
        ...
        {priorities.map((p, i) => {
          ...
          return (
            <button
              ...
              className="w-full text-left p-3 rounded-xl border dark:border-white/10 border-border/70 dark:bg-white/[0.02] bg-white/60 hover:scale-[1.01] transition-transform group space-y-2"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-2 min-w-0 flex-1">
                  <ListChecks className="h-4 w-4 mt-0.5 shrink-0 dark:text-primary text-info" />
                  <div className="min-w-0 flex-1">
                    <div className="font-medium text-sm dark:text-white text-info line-clamp-2">{p.title}</div>
                    <div className="text-xs dark:text-white/50 text-muted-foreground mt-0.5">
                ...
                <ArrowRight className="h-3.5 w-3.5 shrink-0 dark:text-white/30 text-muted-foreground group-hover:translate-x-0.5 transition-transform mt-1" />
              </div>
              <div className="space-y-1">
                <div className="flex items-center justify-between text-[11px] dark:text-white/50 text-muted-foreground">
