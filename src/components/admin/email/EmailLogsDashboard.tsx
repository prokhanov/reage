...
const STATUSES: { value: string; label: string }[] = [
  ...
  { value: "dlq", label: "Ошибка" },
  { value: "failed", label: "Не отправлено" },
];
...
export function EmailLogsDashboard() {
  ...
  const fetchData = async () => {
    ...
    if (!error && data) {
      ...
      setRows(deduped);
      const tmpls = Array.from(new Set(deduped.map((r) => r.template_name).filter(Boolean))).sort();
      setTemplates(tmpls);
    } else {
      setRows([]);
      setTemplates([]);
        ...
        <CardHeader className="pb-3">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <CardDescription>Все отправленные письма системы</CardDescription>
            </div>
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-wrap gap-3">
            ...
            <div className="space-y-1 min-w-[180px]">
              <label className="text-xs text-muted-foreground">Тип письма</label>
