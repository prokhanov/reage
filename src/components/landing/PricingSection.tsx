...
function BiomarkersMetricRow({ biomarkers, biomarkersBySystem, isPopular, label }: { biomarkers: string; biomarkersBySystem: BiomarkerCategory[]; isPopular?: boolean; label?: string }) {
  ...
  return (
    <Popover>
      ...
      {hasData && (
        <PopoverContent className="w-80 sm:w-96 p-4" align="center">
...
const audienceMap: Record<string, { who: string; gain: string }> = {
  basic: {
    who: "Тем, кто хочет начать регулярный мониторинг здоровья и контролировать ключевые показатели",
    gain: "Базовая оценка основных систем, раннее выявление отклонений и отслеживание динамики",
  },
  plus: {
    who: "Тем, у кого есть факторы риска или жалобы, кто хочет детально разобрать сердце и метаболизм",
    gain: "Расширенная диагностика сердечно-сосудистых рисков, гормонов и индивидуальный план здоровья",
  },
  expert: {
    who: "Тем, кто хочет максимально глубоко оценить процессы старения и осознанно управлять биовозрастом",
    gain: "Полный охват систем, приоритетное сопровождение и контроль ключевых маркеров старения",
  },
};
...
export const STARTER_CARD = {
  ...
  description: "",
  who:
    "Тем, кто хочет разово проверить ключевые показатели и познакомиться с подходом ReAge",
  gain:
    "Срез по крови, моче, метаболизму и щитовидной железе с разбором врача и отчётом ReAge",
...
export function PricingSection({ showStarterPlan = false }: { showStarterPlan?: boolean } = {}) {
  ...
  return (
    <section id="pricing" className="relative py-6 md:py-8 scroll-mt-16">
