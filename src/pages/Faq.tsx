...
const SECTIONS: Section[] = [
  {
    ...
    items: [
      ...
      {
        q: "Почему так дорого по сравнению с обычным чекапом?",
      ...
      {
        q: "Какой минимальный возраст для участия в программе?",
        a: "С 18 лет.",
      },
    ],
  },
  ...
  },
  {
    ...
    items: [
      {
        q: "Я регулярно сдаю анализы сам — зачем мне ReAge?",
        a: "Лаборатория показывает отдельные цифры. ReAge собирает их в единую картину и показывает, как они меняются год к году — то, что отдельные анализы никогда не покажут.",
      },
      {
        q: "Почему нельзя просто сдавать анализы в обычной лаборатории?",
        a: "Во-первых, это будет значительно дороже.\n\nВо-вторых, это даст лишь отдельные результаты без интерпретации и без сравнения с предыдущей точкой.\n\nКроме того, «в пределах нормы» не всегда означает, что всё в порядке. Лабораторные референсные значения формируются на основе статистики большой группы людей — в эти диапазоны попадают и показатели тех, у кого уже есть заболевания. Поэтому формальное соответствие норме не гарантирует оптимальное состояние организма.\n\nПлюс в обычном чекапе показатели обычно смотрят изолированно — каждый отдельно от других. Но организм работает как единая система: без анализа взаимосвязей между показателями вы видите цифры, но не понимаете, что на самом деле происходит и какие риски остаются незаметными, даже если каждый отдельный анализ «в норме».",
      },
      {
        q: "У меня уже куча анализов, я не понимаю, что с ними делать — это про меня?",
        a: "Да. Основная проблема не в нехватке анализов, а в том, что цифры без контекста ничего не говорят. ReAge собирает их в одну картину и объясняет, что это значит именно для вас.",
      },
      {
        q: "Что если через год окажется, что всё то же самое, ничего не изменилось?",
        a: "Это тоже ценная информация — значит, текущий образ жизни поддерживает стабильное состояние, и это подтверждено данными, а не предположением.",
      },
    ],
  },
  {
    ...
    items: [
      ...
      {
        q: "Я чувствую себя отлично — мне это рано?",
        a: "Многие изменения в организме происходят задолго до симптомов. Чем раньше есть отправная точка, тем раньше видна тенденция, а не только факт, когда уже что-то болит.",
      },
      {
